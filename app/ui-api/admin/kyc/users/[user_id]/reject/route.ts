...
export async function POST(req: NextRequest, ctx: { params: Promise<{ user_id: string }> }) {
  try {
    ...
    const res = await fetch(`${API_BASE}/admin/kyc/users/${encodeURIComponent(user_id)}/reject${qs}`, {
      method: "POST",
    ...
    return NextResponse.json({ detail: err?.message || "KYC reject proxy failed" }, { status: 500 });
  }
}
