...
file = Path("/var/www/coin2win-ui/app/bets/page.tsx")
      ...
      <div style={wrapStyle}>
        ...
        <div style={toolbarStyle}>
          ...
          <div style={filterWrapStyle}>
            {(["all", "dice", "crash"] as const).map((value) => (
              <button
                key={value}
                onClick={() => setGameFilter(value)}
                style={{
                  ...filterButtonStyle,
                  background: gameFilter === value ? "#00e701" : "rgba(255,255,255,0.06)",
                  color: gameFilter === value ? "#071824" : "#dbe7ee",
                }}
              >
                {value === "all" ? "All" : value === "dice" ? "Dice" : "Crash"}
              </button>
            ))}
          </div>
        </div>
